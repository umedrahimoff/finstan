...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const sql = neon(url)
    const rows = await sql`SELECT id, username, password_hash, role, frozen, tenant_id FROM app_users WHERE username = ${u} LIMIT 1`
    const user = rows[0] as { id: string; username: string; password_hash: string; role?: string; frozen?: boolean; tenant_id?: string | null } | undefined
